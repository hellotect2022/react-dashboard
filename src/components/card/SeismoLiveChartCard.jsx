...
const SeismoLiveChartCard = forwardRef((props, ref) => {
    ...
    useEffect(() => {
        const disconnect = sseAPI.connect(
            '/sse/stream/seismo',
            (data) => {
                console.log('📊 Seismo 데이터 수신:', data);
                updateStreamData(data);
                setIsConnected(true);
            },
            (error) => {
                console.error('❌ Seismo SSE 연결 에러:', error);
    ...
    // 동적으로 라인 생성 (데이터 구조에 따라 수정 필요)
    const dynamicLines = useMemo(() => {
        if (sseDataList.length === 0) return [];

        const firstEntry = sseDataList[0];
        // seismo 데이터 구조에 맞게 수정
        // 예: { x: 1.2, y: 0.5, z: 0.8 } 형태라면
        if (firstEntry.value) {
            const keys = Object.keys(firstEntry.value).filter(key => typeof firstEntry.value[key] === 'number');
        ...
        <CardLayout
            ...
            title={
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-medium text-[20px]">{props.title || 'Seismo'}</span>
...
SeismoLiveChartCard.displayName = 'SeismoLiveChartCard';

export default SeismoLiveChartCard;
